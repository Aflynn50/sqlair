...
type TxRunner func(context.Context, func(context.Context, *sqlair.TX) error) error
...
CREATE TABLE agent (
	...
	create_index1 = `CREATE INDEX idx_agent_model_name ON agent (model_name);`
	create_index2 = `CREATE INDEX idx_agent_status ON agent (status);`
...
CREATE TABLE agent_events (
	...
	create_index3 = `CREATE INDEX idx_agent_events_event ON agent_events (event);`
)

var schemas = []string{create_agent, create_index1, create_index2, create_agent_events, create_index3}
...
func SlotAllDBBasedTransactionRunner(slots int) func(*sqlair.DB) TxRunner {
	slotCh := make(chan any, slots)

	return func(db *sqlair.DB) TxRunner {
		txnRunner := sqlairtxn.NewTransactionRunner()

		return TxRunner(func(c context.Context, fn func(context.Context, *sqlair.TX) error) error {
			slotCh <- nil
			defer func() { <-slotCh }()
			err := txnRunner.Retry(c, func() error {
				return txnRunner.Txn(c, db, fn)
			})
			return err
		})
	}
...
func SingleConnectionTransactionRunner(db *sqlair.DB) TxRunner {
	db.PlainDB().SetMaxOpenConns(1)

	return TxRunner(func(c context.Context, fn func(context.Context, *sqlair.TX) error) error {
		tx, err := db.Begin(c, nil)
		if err != nil {
			return err
		}
...
func RetryableTransactionRunner(db *sqlair.DB) TxRunner {
	txnRunner := sqlairtxn.NewTransactionRunner()
