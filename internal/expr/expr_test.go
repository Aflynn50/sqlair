...
func (s *ExprSuite) TestPrepareErrors(c *C) {
	...
	tests := []struct {
		...
		prepareArgs: []any{Person{}},
		err:         `cannot prepare statement: cannot use sql function with asterisk in output expression: "avg(num) AS &Person.*"`,
	}, {
		query:       "SELECT avg(id) AS &M.* FROM t",
		prepareArgs: []any{sqlair.M{}},
		err:         `cannot prepare statement: cannot use sql function with asterisk in output expression: "avg(id) AS &M.*"`,
	}, {
